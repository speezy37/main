...
** 2a1. Address book displays message “No schedule allocated”.
+
Use case ends.
...
. User enters setschedule command.
. AddressBook prompts to add schedule.
. User enters the details of the schedule.
. AddressBook adds the schedule.
...
* 4a. Target is not user and user is not ADMINISTRATOR priority level.
** 4a1. AddressBook displays message "You must have a priority level of at least ADMINISTRATOR to perform this
operation.".
+
Use case ends.
...
==== List schedule
. Lists the schedule of an employee in address book.

.. Prerequisites: Logged in as an any user with basic priority level with index 4. +
.. Test case: `schedule 4` +
Expected: `displays schedule of employee at index 4`.
.. Test case: `schedule 0` +
Expected: Index 0 is invalid. Error details shown in status message.

==== Set schedule
. Sets the schedule of an employee in address book.

.. Prerequisites: Logged in as an any user with basic priority level with index 4. +

.. Test case: `setschedule 4 ts/0900 te/1700 v/Level 5` +
Expected: Schedule set for employee at index 4. Command success is shown in the status message.
.. Test case: `setschedule 4 ts/0900 te/0900 v/Level 5` +
Expected: Schedule not set. Time start and time end are the same. Error detail shown in the status message.
