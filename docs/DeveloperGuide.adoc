...
. User types the command `login ic/NRIC pwd/PASSWORD`.
. AddressBook shows welcome message upon successul login.
+
Use case ends.
...
** 2a1. AddressBook shows an error message stating that command format is incorrect.
+
Use case ends.

* 2b. Format of login command is correct, but invalid NRIC and/or password.
** 2b1. AddressBook shows error message: - +
`Login failed. Incorrect NRIC and/or password.`
+
Use case ends.
...
. User enters command `edit` followed by editable fields.
...
`You must have a priority level of at least ADMINISTRATOR to perform this operation.`
+
Use case ends.
...
** 2a1. "0 leaves listed." Error shown by AddressBook. +
User case ends.
...
[NOTE]
====
Refer to <<UserGuide.adoc#editing-a-person-code-edit-code,EditCommand User Guide>> for the list of editable parameters.
====
...
...     The following test cases assume that the employee list is *neither sorted nor filtered* prior to executing the
            test cases.
Modify your test cases accordingly if you have sorted the list
