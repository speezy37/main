...
** Cons: The command may not work properly. For example, there is an employee whose name is `Junior` from `Senior Department`. But if we only want to filter employees who are in `Junior Management`, the command will output people who are in `Junior Management` and the employee `Junior`.
