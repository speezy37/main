...
[discrete]
=== Use case: Apply leave
*MSS*

.	User login to BankAB.
.	User enters `leave` command with the desired leave application date.
.	BankAB adds the leave application to the database for approval.
+
Use case ends.

*Extensions*

* 2a. The given date is an invalid calendar date.
** 2a1. BankAB shows an error message.
+
Use case resumes at step 2.

* 2b. The given date has passed.
** 2b1. BankAB shows an error message.
+
Use case resumes at step 2.

* 2c. The given date is an existing leave application.
** 2c1. BankAB shows an error message.
+
Use case resumes at step 2.
...
.	BankAB list all leave applications
+
Use case ends.

[discrete]
=== Use case: Filter leave

*MSS*

.	User enters `filterleave` command with the desired NRIC he wants to filter
.	BankAB list all leave applications under that NRIC
+
Use case ends.

*Extensions*

* 1a. Invalid NRIC is given for `filterleave` command.
+
Use case resumes at step 1.

[discrete]
=== Use case: Delete leave
*MSS*

.	User login to BankAB.
.	User enters `deleteleave` command with the index of the leave application he wants to delete.
.	BankAB deletes the leave application from the database.
+
Use case ends.

*Extensions*

* 2a. Invalid index given.
** 2a1. BankAB shows an error message. +
Use case end.
* 2b. Not authorized to delete other user's leave application due to insufficient priority level.
** 2b1. BankAB shows an error message.
+
Use case end.

[discrete]
=== Use case: Approve/reject leave request
*MSS*

.	User login to BankAB.
.	User enters `approve` / `reject` command with the index of the leave application he wants to approve.
.	BankAB approves the leave application.
+
Use case ends.
...
** 1a1. BankAB shows an error message.
+
Use case ends.
...
* 2b. Not authorized to approve/reject for own leave application.
** 2b1. BankAB shows an error message.
+
Use case end.

* 2c. Not authorized to approve/reject other user's leave application due to insufficient priority level.
** 2c1. BankAB shows an error message.
+
Use case end.

* 2d. The leave application was already approved/rejected.
** 2d1. BankAB shows an error message.
+
...
.. Test case: `listleave` +
   Expected: All leave applications will be listed.
.. Test case: 'listleave 1` +
   Expected: All leave applications will be listed.
.. Other extra characters after 'listleave' such as: `listleave 2 2`, `listleave **` +
   Expected: All leave applications will be listed.
...
.. Prerequisites: User must be logged in as Administrator.
.. Test case: `deleteleave 1` where the leave application under index 1 is not requested by the logged in user. +
...
.. Other incorrect deleteleave commands to try: `deleteleave`, `deleteleave x` (where x is larger than the list leave size) _{give more}_ +
   Expected: Similar to previous.
...
.. Prerequisites: User must be logged in as Administrator.
.. Test case: `approve 1` where the leave application under index 1 is not requested by the logged in user. +
...
.. Other incorrect approve commands to try: `approve`, `approve x` (where x is larger than the list leave size) _{give more}_ +
   Expected: Similar to previous.
...
.. Prerequisites: User must be logged in as Administrator.
.. Test case: `reject 1` where the leave application under index 1 is not requested by the logged in user. +
...
.. Other incorrect reject commands to try: `reject`, `reject x` (where x is larger than the list leave size) _{give more}_ +
   Expected: Similar to previous.
